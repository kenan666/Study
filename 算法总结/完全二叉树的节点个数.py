...
'''
'''
对于有n层的满二叉树（根节点算第一层），其节点数为2^n-1个，第i层的节点数为2^(i-1) ，而其潜在的空节点（也就是第n+1层）有2^n 个，
刚好是树的节点数+1，换句话说，树的节点数是空节点-1.

推广到一般例子，在n层的满二叉树的基础上（节点数2^n-1，空节点2^n），生长一个节点，则节点数+1，空节点数-1然后+2，结果还是+1；
这就保障了空节点数是节点数+1的关系。
