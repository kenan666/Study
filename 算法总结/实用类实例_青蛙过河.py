...
接下来，依次遍历每个位置上的石头。对于每个 currentPosition，遍历 value 中每个 jumpsize，判断位置 currentPosition + newjumpsize 是否存在于 mapmap 中，
