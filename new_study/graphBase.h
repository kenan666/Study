...
{ 
	double x; 
	double y; 
	POINT_EX(double a=0, double b=0) { x=a; y=b;} //constructor 
}; 
struct LINESEG   // 嵌套结构体
{ 
	POINT_EX s; 
	POINT_EX e; 
	LINESEG(POINT_EX a, POINT_EX b) { s=a; e=b;} 
	LINESEG() { } 
}; 
...
double dist(POINT_EX p1,POINT_EX p2)                // 返回两点之间欧氏距离 
{ 
	return( sqrt( (p1.x-p2.x)*(p1.x-p2.x)+(p1.y-p2.y)*(p1.y-p2.y) ) ); 
} 
bool equal_point(POINT_EX p1,POINT_EX p2)           // 判断两个点是否重合  
{ 
...
r<0：ep在矢量opsp的顺时针方向 
*******************************************************************************/ 
double multiply(POINT_EX sp,POINT_EX ep,POINT_EX op) 
{ 
...
r>0：两矢量夹角为钝角 
*******************************************************************************/ 
double dotmultiply(POINT_EX p1,POINT_EX p2,POINT_EX p0) 
{ 
...
*******************************************************************************/ 
bool online(LINESEG l,POINT_EX p) 
{ 
	return( (multiply(l.e,p,l.s)==0) &&( ( (p.x-l.s.x)*(p.x-l.e.x)<=0 )&&( (p.y-l.s.y)*(p.y-l.e.y)<=0 ) ) ); 
} 
// 返回点p以点o为圆心逆时针旋转alpha(单位：弧度)后所在的位置 
POINT_EX rotate(POINT_EX o,double alpha,POINT_EX p) 
{ 
	POINT_EX tp; 
	p.x-=o.x; 
	p.y-=o.y; 
...
原理：
	...
	-1<r<1 && r'<=0	angle = -arccos(r)
*/ 
double angle(POINT_EX o,POINT_EX s,POINT_EX e) 
{ 
...
*/ 
double relation(POINT_EX p,LINESEG l) 
{ 
	LINESEG tl; 
	tl.s=l.s; 
	tl.e=p; 
...
// 求点C到线段AB所在直线的垂足 P 
POINT_EX perpendicular(POINT_EX p,LINESEG l) 
{ 
...
double ptolinesegdist(POINT_EX p,LINESEG l,POINT_EX &np) 
{ 
	double r=relation(p,l); 
	if(r<0) 
	{ 
...
double ptoldist(POINT_EX p,LINESEG l) 
{ 
	return fabs(multiply(p,l.e,l.s))/dist(l.s,l.e); 
} 
/* 计算点到折线集的最近距离,并返回最近点. 
注意：调用的是ptolineseg()函数 */ 
double ptopointset(int vcount,POINT_EX pointset[],POINT_EX p,POINT_EX &q) 
{ 
	int i; 
...
bool CircleInsidePolygon(int vcount,POINT_EX center,double radius,POINT_EX polygon[]) 
{ 
	POINT_EX q; 
	double d; 
	q.x=0; 
...
LINE makeline(POINT_EX p1,POINT_EX p2) 
{ 
	LINE tl; 
	int sign = 1; 
	tl.a=p2.y-p1.y; 
...
// 求点p关于直线l的对称点  
POINT_EX symmetry(LINE l,POINT_EX p) 
{ 
   POINT_EX tp; 
...
bool lineintersect(LINE l1,LINE l2,POINT_EX &p) // 是 L1，L2 
{ 
...
bool intersection(LINESEG l1,LINESEG l2,POINT_EX &inter) 
{ 
	LINE ll1,ll2; 
...
本程序默认第一个条件已经满足 
*/ 
bool issimple(int vcount,POINT_EX polygon[]) 
{ 
	int i,cn; 
	LINESEG l1,l2; 
	for(i=0;i<vcount;i++) 
...
void checkconvex(int vcount,POINT_EX polygon[],bool bc[]) 
{ 
	int i,index=0; 
	POINT_EX tp=polygon[0]; 
...
bool isconvex(int vcount,POINT_EX polygon[]) 
{ 
	bool bc[MAXV]; 
...
double area_of_polygon(int vcount,POINT_EX polygon[]) 
{ 
	int i; 
	double s; 
	if (vcount<3) 
...
// 如果输入顶点按逆时针排列，返回true 
bool isconterclock(int vcount,POINT_EX polygon[]) 
{ 
	return area_of_polygon(vcount,polygon)>0; 
} 
// 另一种判断多边形顶点排列方向的方法  
bool isccwize(int vcount,POINT_EX polygon[]) 
{ 
	int i,index; 
	POINT_EX a,b,v; 
	v=polygon[0]; 
...
射线法判断点q与多边形polygon的位置关系，要求polygon为简单多边形，顶点逆时针排列 
   ...
   如果点在多边形外：	返回2 
*********************************************************************************************/ 
int insidepolygon(int vcount,POINT_EX Polygon[],POINT_EX q) 
{ 
	int c=0,i,n; 
	LINESEG l1,l2; 
...
bool InsideConvexPolygon(int vcount,POINT_EX polygon[],POINT_EX q) // 可用于三角形！ 
{ 
	POINT_EX p; 
	LINESEG l; 
	int i; 
...
len为输出的凸包上的点的个数 
**********************************************/ 
void Graham_scan(POINT_EX PointSet[],POINT_EX ch[],int n,int &len) 
{ 
	int i,j,k=0,top=2; 
	POINT_EX tmp; 
...
void ConvexClosure(POINT_EX PointSet[],POINT_EX ch[],int n,int &len) 
{ 
...
**********************************************************************************************/ 
bool LinesegInsidePolygon(int vcount,POINT_EX polygon[],LINESEG l) 
{ 
	// 判断线端l的端点是否不都在多边形内 
	if(!insidepolygon(vcount,polygon,l.s)||!insidepolygon(vcount,polygon,l.e)) 
		return false; 
	int top=0,i,j; 
	POINT_EX PointSet[MAXV],tmp; 
	LINESEG s; 
...
POINT_EX cg_simple(int vcount,POINT_EX polygon[]) 
{ 
...
POINT_EX gravitycenter(int vcount,POINT_EX polygon[]) 
{ 
	POINT_EX tp; 
...
结论	： x坐标最大，最小的点肯定是凸顶点 
	y坐标最大，最小的点肯定是凸顶点            
************************************************/ 
POINT_EX a_point_insidepoly(int vcount,POINT_EX polygon[]) 
{ 
	POINT_EX v,a,b,r; 
	int i,index; 
	v=polygon[0]; 
...
/***********************************************************************************************/ 
void pointtangentpoly(int vcount,POINT_EX polygon[],POINT_EX p,POINT_EX &rp,POINT_EX &lp) 
{ 
	LINESEG ep,en; 
	bool blp,bln; 
	rp=polygon[0]; 
...
bool core_exist(int vcount,POINT_EX polygon[],POINT_EX &p) 
{ 
	int i,j,k; 
	LINESEG l; 
	LINE lineset[MAXV]; 
...
用途	： 因为圆为凸集，所以判断点集，折线，多边形是否在圆内时，
	只需要逐一判断点是否在圆内即可。 
*******************************************************************************/ 
bool point_in_circle(POINT_EX o,double r,POINT_EX p) 
{ 
...
*******************************************************************************/ 
bool cocircle(POINT_EX p1,POINT_EX p2,POINT_EX p3,POINT_EX &q,double &r) 
{ 
...
int line_circle(LINE l,POINT_EX o,double r,POINT_EX &p1,POINT_EX &p2) 
{ 
	return true; 
} 
...
POINT_EX rect4th(POINT_EX a,POINT_EX b,POINT_EX c) 
{ 
	POINT_EX d; 
...
内含： return 5； 
*/ 
int CircleRelation(POINT_EX p1, double r1, POINT_EX p2, double r2) 
{ 
...
// 调用ptoldist函数，在第4页 
bool CircleRecRelation(POINT_EX pc, double r, POINT_EX pr1, POINT_EX pr2, POINT_EX pr3, POINT_EX pr4) 
{ 
...
//点是否在直线同侧：
//两个点是否在直线同侧，是则返回true 
bool SameSide(POINT_EX p1, POINT_EX p2, LINE line) 
{ 
...
//两圆交点： 
// 两圆已经相交（相切） 
void  c2point(POINT_EX p1,double r1,POINT_EX p2,double r2,POINT_EX &rp1,POINT_EX &rp2) 
{ 
	double a,b,r; 
	a=p2.x-p1.x; 
	b=p2.y-p1.y; 
...
//两圆公共面积：
// 必须保证相交 
//double c2area(POINT_EX p1,double r1,POINT_EX p2,double r2) 
//{ 
//	POINT_EX rp1,rp2; 
...
//圆和直线关系： 
//0----相离 1----相切 2----相交 
int clpoint(POINT_EX p,double r,double a,double b,double c,POINT_EX &rp1,POINT_EX &rp2) 
{ 
	int res=0; 
...
//内切圆： 
void incircle(POINT_EX p1,POINT_EX p2,POINT_EX p3,POINT_EX &rp,double &r) 
{ 
...
void cutpoint(POINT_EX p,double r,POINT_EX sp,POINT_EX &rp1,POINT_EX &rp2) 
{ 
	POINT_EX p2; 
	p2.x=(p.x+sp.x)/2; 
	p2.y=(p.y+sp.y)/2; 
